...
// Window function calculation over a sorted array of numbers
...
// Partition data by a field (Pane)
...
export default function WindowFunctionDashboard() {
  ...
  // Partition data into panes
  const panes = useMemo(() => partitionData(sortedData, partitionBy), [sortedData, partitionBy]);

  // Compute window calculations for each pane
  // For simplicity, window moves "Across" means over sorted quarters within pane
  // "Down" means over panes vertically (not implemented fully here, but scaffolded)
  const panesWithWindowCalc = useMemo(() => {
    const result = new Map<string, (DataItem & { windowValue: number | null })[]>();

    panes.forEach((items, key) => {
      // Sort items by Year & Quarter for Across direction
      const sortedItems = [...items].sort(
        (a, b) => a.Year - b.Year || quarterOrder[a.Quarter] - quarterOrder[b.Quarter]
      );

      // Extract values to calculate window metric on (e.g. Sales)
      const values = sortedItems.map((d) => d.Sales);

      // Calculate window metric for each item
  ...
  return (
    <div style={{ padding: 20, fontFamily: "Arial, sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 20, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <label>
          Direction:
          <select value={direction} onChange={(e) => setDirection(e.target.value as "Across" | "Down")}>
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        {[...panesWithWindowCalc.entries()].map(([paneKey, items]) => (
          ...
          >
            ...
            {/* Chart showing windowValue over time */}
