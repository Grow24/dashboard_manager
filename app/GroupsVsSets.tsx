...
function App() {
  // Groups: array of { id, name, categories: [] }
  const [groups, setGroups] = useState([
    { id: "g1", name: "Group 1", categories: ["A", "B"] },
    { id: "g2", name: "Group 2", categories: ["C", "D", "E"] },
  ]);

  // Sets: array of { id, name, type: "top"|"bottom"|"manual", count?, categories? }
  const [sets, setSets] = useState([
    { id: "s1", name: "Top 3", type: "top", count: 3 },
    { id: "s2", name: "Bottom 2", type: "bottom", count: 2 },
    { id: "s3", name: "Manual Set", type: "manual", categories: ["F", "G"] },
  ]);

  // Selection: { type: "group"|"set", id: string } or null
  const [selection, setSelection] = useState(null);

  // Hover info for popup
  const [hoverInfo, setHoverInfo] = useState(null);

  // Map category to group id for quick lookup
  ...
  // Compute set categories based on type and data
  const computedSets = useMemo(() => {
    const sorted = [...sampleData].sort((a, b) => b.value - a.value);
    const result = {};
    sets.forEach((set) => {
      if (set.type === "top") {
        result[set.id] = sorted.slice(0, set.count).map((d) => d.category);
      } else if (set.type === "bottom") {
        result[set.id] = sorted.slice(-set.count).map((d) => d.category);
  ...
  // Get selected categories based on selection
  ...
  // Add group with prompt
  const addGroup = () => {
    const name = prompt("Enter group name:");
    if (!name) return;
    const catsStr = prompt(
      "Enter categories (comma separated), e.g. A,B,C"
    );
    if (!catsStr) return;
    const cats = catsStr
      .split(",")
      .map((c) => c.trim())
      .filter((c) => sampleData.some((d) => d.category === c));
    if (cats.length === 0) {
    ...
    setGroups((gs) => [
      ...gs,
      { id: `g${Date.now()}`, name, categories: cats },
    ]);
  };
  ...
  const removeGroup = (id) => {
    ...
    if (selection?.type === "group" && selection.id === id) setSelection(null);
  };
  ...
  const addSet = () => {
    ...
    if (type === "manual") {
      ...
      setSets((ss) => [
        ...ss,
        { id: `s${Date.now()}`, name, type, categories: cats },
      ]);
    } else {
      const countStr = prompt("Enter count (number of categories):");
      const count = parseInt(countStr, 10);
      if (isNaN(count) || count < 1 || count > sampleData.length) {
        alert("Invalid count.");
        return;
      }
      setSets((ss) => [...ss, { id: `s${Date.now()}`, name, type, count }]);
    }
  };
  ...
  const Bar = ({ category, value }) => {
    const maxVal = Math.max(...sampleData.map((d) => d.value));
    const width = (value / maxVal) * barMaxWidth;
    const opacity = getOpacity(category);
    const groupId = categoryToGroupId[category];
    const group = groups.find((g) => g.id === groupId);
    const inSelectedSet = selection?.type === "set" && computedSets[selection.id]?.includes(category);
  ...
  return (
    ...
    >
      <h1 style={{ textAlign: "center", marginBottom: 20 }}>
        Groups vs Sets (Standard Implementation)
      </h1>
      ...
      <div style={{ display: "flex", gap: 40 }}>
        ...
        <div style={{ flex: 1 }}>
          <h2>Data Bars</h2>
          {sampleData.map((d) => (
            <Bar key={d.category} category={d.category} value={d.value} />
          ))}
        </div>
        ...
        <div style={{ flex: 1 }}>
          <h2>Groups</h2>
          <button onClick={addGroup} style={{ marginBottom: 8 }}>
            + Add Group
          </button>
          {groups.length === 0 && (
          ...
          <button onClick={addSet} style={{ marginBottom: 8 }}>
            + Add Set
          </button>
          {sets.length === 0 && (
            <div style={{ fontStyle: "italic", color: "#888" }}>
          ...
          {sets.map(({ id, name, type, count, categories }) => (
            <div
              key={id}
              style={{
                border:
        ...
        >
          {hoverInfo.category && (
            <>
              ...
              <div>
                <strong>Value:</strong>{" "}
                {sampleData.find((d) => d.category === hoverInfo.category)
                  ?.value || ""}
              </div>
              <div>
