...
// const { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, Legend, CartesianGrid, PieChart, Pie, Cell } = (window as any).Recharts || {};
...
const App: React.FC = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [salesRes, inquiryRes] = await Promise.all([
        fetch('https://intelligentsalesman.com/ism1/API/sales_data.php'),
        fetch('https://intelligentsalesman.com/ism1/API/sales_data.php?data_type=inquiry'),
      ]);

      if (!salesRes.ok) throw new Error(`Sales fetch failed: ${salesRes.statusText}`);
      if (!inquiryRes.ok) throw new Error(`Inquiry fetch failed: ${inquiryRes.statusText}`);

      const [salesJson, inquiryJson] = await Promise.all([salesRes.json(), inquiryRes.json()]);
      setSalesData(Array.isArray(salesJson) ? salesJson : []);
      setInquiryData(Array.isArray(inquiryJson) ? inquiryJson : []);
    } catch (e: any) {
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      if (isRanking) {
        // Ranking mode: write result into inquiryData
        ...
        const response = await fetch('https://intelligentsalesman.com/ism1/API/api/executeRankingQuery.php', {
          method: 'POST',
        ...
        // Refresh sales with simple filters
        const params: any = {};
        ...
        const salesQS = new URLSearchParams(params).toString();
        const salesRes = await fetch(`https://intelligentsalesman.com/ism1/API/sales_data.php?${salesQS}`);
        const salesJson = await salesRes.json();
        setSalesData(Array.isArray(salesJson) ? salesJson : []);
      } else {
        // Normal mode: call both APIs with the same params
        const params: any = {};
        selectedWithoutQuery.forEach((filter) => {
          const val = filterValues[filter.id];
          if (val && val !== '' && !(Array.isArray(val) && val.length === 0)) {
        ...
        const [salesRes, inquiryRes] = await Promise.all([
          fetch(`https://intelligentsalesman.com/ism1/API/sales_data.php?${queryString}`),
          fetch(`https://intelligentsalesman.com/ism1/API/sales_data.php?data_type=inquiry&${queryString}`),
        ]);

        if (!salesRes.ok) throw new Error(`Error fetching sales data: ${salesRes.statusText}`);
        if (!inquiryRes.ok) throw new Error(`Error fetching inquiry data: ${inquiryRes.statusText}`);
  ...
  // KPIs & charts are based on salesData only
  ...
  // Pie chart: Distribution by Status (sales status)
  ...
  // Monthly overview: sum of salesAmount by YYYY-MM
  ...
  return (
    <div className="p-6 mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">KPIs and charts reflect Sales data. The last table shows Inquiry data.</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div className="text-lg font-semibold text-gray-900">Inquiry List</div>
          <div className="text-sm text-gray-600">
            Showing {inquiryData.length} results
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loadingData ? (
                <tr><td colSpan={9} className="text-center py-6">Loading data...</td></tr>
              ) : dataError ? (
                <tr><td colSpan={9} className="text-center py-6 text-red-600">{dataError}</td></tr>
              ) : inquiryData.length === 0 ? (
                <tr><td colSpan={9} className="text-center py-6">No inquiry data</td></tr>
              ) : (
                inquiryData.map((row: any, index: number) => (
                  <tr key={row.inquiryId ?? index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{row.inquiryId}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.accountName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.inqOwner}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
                        {row.inquiryStatus}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.territoryName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.sheetName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.ourPotential}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.quotationStatus}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.updatedTime}</td>
                  </tr>
                ))
              )}
            </tbody>
